(() => {
  ...
  // ==========================================
  // 1. ΚΕΝΤΡΙΚΕΣ ΡΥΘΜΙΣΕΙΣ (CONFIG)
  // ==========================================
  const CONFIG = Object.freeze({
    oceanAudioUrl: "https://grafis.sch.gr/index.php/s/fMEcaLWSZxManpy/download",
    groups: {
      A: ['peristasi', 'platamonas', 'pydna', 'methoni', 'kolindros', 'olympus', 'enipeas', 'agia_triada', 'elatochori', 'katerini_park', 'olympiaki_akti', 'dion_museum', 'monastery'],
      B: ['skot', 'moni_makryrrachis', 'korinos_tombs', 'leivithra', 'kountouriotissa', 'olympus_center', 'alyki', 'poroi', 'platanodasos', 'orlias']
    },
    // Ομαδοποίηση όλων των SVG Animations! Τέλος το χαοτικό getElementById.
    svg: {
      monsters: ['kraken-monster', 'stym-monster', 'sirens-monster', 'chimera-monster', 'echidna-monster', 'charybdis-monster', 'harpies-monster', 'cerberus-monster', 'scylla-monster', 'centaurs-monster', 'cetus-monster', 'aloadae-monster', 'typhon-monster', 'macedonian-lion-monster', 'telchines-monster', 'talos-monster', 'cyclops-monster', 'medusa-monster', 'calydonian-boar-monster', 'lycaon-monster', 'minotaur-monster', 'charon-monster', 'bronze-bull-monster', 'arachne-monster', 'ultimate-storm-monster', 'pegasus-monster', 'sphinx-monster', 'colchian-monster'],
      eco: ['eagle-anim', 'flamingos-anim', 'chamois-anim', 'turtle-anim', 'bear-anim', 'otter-anim', 'boar-anim-new', 'woodpecker-nature', 'roe-deer-nature', 'salamander-anim', 'fox-eco-nature', 'pelican-eco-nature', 'tritonas-eco-nature', 'nerofido-eco-nature', 'crab-sea-nature', 'dolphins-sea-nature', 'avocet-anim', 'dipper-anim', 'gull-sea-nature', 'school-sea-nature', 'seal-sea-nature', 'alkyoni-eco-nature', 'squirrel-eco-nature', 'mustang-eco-nature', 'owl-eco-nature', 'firefly-eco-nature', 'hedgehog-eco-nature', 'apollo-eco-nature', 'bat-eco-nature', 'frog-eco-nature', 'hoopoe-eco-nature', 'hawk-eco-nature', 'hare-eco-nature']
    },
    // Φίλτρα Περιβάλλοντος
    atmospheres: {
      'dark-mode': ['mythic_hades', 'skot', 'mythic_typhon', 'mythic_echidna', 'mythic_storm', 'mythic_lycaon', 'mythic_pydna_fate', 'mythic_cerberus', 'mythic_charon', 'mythic_centaurs', 'mythic_hydra', 'mythic_lions'],
      'abyss-mode': ['mythic_cetus', 'mythic_charybdis', 'mythic_kraken', 'mythic_telchines', 'mythic_scylla'],
      'fx-snow': ['elatochori', 'olympus', 'eco_rompolo'],
      'fx-sparkles': ['mythic_muses', 'mythic_orpheus', 'mythic_methoni_alphabet'],
      'fx-lightning': ['mythic_zeus', 'mythic_storm', 'mythic_harpies'],
      'petrified-map': ['mythic_medusa']
    },
    monsterPlaceIDs: ['mythic_kraken', 'mythic_chimera', 'mythic_echidna', 'mythic_charybdis', 'mythic_harpies', 'mythic_cerberus', 'mythic_scylla', 'mythic_centaurs', 'mythic_cetus', 'mythic_typhon', 'mythic_hydra', 'mythic_telchines', 'mythic_lions', 'mythic_stymphalian']
  });
  ...
  // ==========================================
  // 2. Η ΒΑΣΗ ΔΕΔΟΜΕΝΩΝ (Η μυθολογία/οικολογία σου)
  // ==========================================
  window.MYTHIC_DB = {
    peristasi: { 
...
mythic_echidna: { 
  ...
  // ==========================================
  // 3. AUDIO ENGINE (Κεντρικός Έλεγχος Ήχου)
  // ==========================================
  const AudioEngine = {
   toggleMute: () => {
      STATE.isSoundOn = !STATE.isSoundOn;
      const btn = document.getElementById('sound-btn-2');
      if (btn) btn.innerHTML = STATE.isSoundOn ? "🔊" : "🔇";

      if (!STATE.isSoundOn) {
        AudioEngine.stopAll();
      } else {
        // Επαναφορά της χαλαρωτικής ατμόσφαιρας μόλις ο χρήστης ενεργοποιήσει ξανά τον ήχο
        AudioEngine.playOcean();
      }
    },
    stopAll: () => {
      clearTimeout(STATE.audioTimers.stop);
      clearInterval(STATE.audioTimers.fade);
      if (STATE.audioOcean) { STATE.audioOcean.pause(); STATE.audioOcean.currentTime = 0; }
      if (STATE.audioCurrentPin) { STATE.audioCurrentPin.pause(); STATE.audioCurrentPin.currentTime = 0; }
    },
   playOcean: () => {
      if (!STATE.isSoundOn) return;
      
      // Διακόπτουμε ΜΟΝΟ την αφήγηση και τα timers, χωρίς να μηδενίσουμε τον ήχο της θάλασσας
      clearTimeout(STATE.audioTimers.stop);
      clearInterval(STATE.audioTimers.fade);
      if (STATE.audioCurrentPin) { STATE.audioCurrentPin.pause(); STATE.audioCurrentPin.currentTime = 0; }

      STATE.audioOcean.volume = 1.0;
      // Παίζουμε τη θάλασσα μόνο αν δεν παίζει ήδη
      if (STATE.audioOcean.paused) {
        STATE.audioOcean.play().catch(() => {});
      }

    
      STATE.audioTimers.stop = setTimeout(() => {
        let fadeSteps = 10; // Ανεξάρτητος μετρητής, χωρίς να διαβάζουμε το hardware volume
        STATE.audioTimers.fade = setInterval(() => {
          fadeSteps--;
          try { STATE.audioOcean.volume = fadeSteps / 10; } catch (e) {} // Αγνοούμε το σφάλμα στο iOS
 ...
 playPin: (url) => {
      if (!STATE.isSoundOn) return;
      AudioEngine.stopAll(); // Πρώτα σταματάμε τον τρέχοντα ήχο, ΟΤΙ ΚΑΙ ΑΝ ΣΥΜΒΕΙ
      if (!url) return;      // Έπειτα ελέγχουμε αν υπάρχει νέος ήχος για αναπαραγωγή

      // Επαναχρησιμοποιούμε το ίδιο αντικείμενο Ήχου (Ασφάλεια μνήμης για κινητά)
      if (!STATE.audioCurrentPin) {
        STATE.audioCurrentPin = new Audio();
        STATE.audioCurrentPin.volume = 0.8;
      }
      STATE.audioCurrentPin.src = url;
      STATE.audioCurrentPin.play().catch(() => {});
    }
  };

  // ==========================================
  // 4. MAP ENGINE (Διαχείριση Κύκλων / Animations)
  // ==========================================
  const MapEngine = {
    cycleState: (isInitialLoad = false) => {
      TooltipEngine.hide();
      if (!isInitialLoad) AudioEngine.playOcean();

      const btn = document.getElementById('toggle-btn-2');
      const monstersBtn = document.getElementById('monsters-only-btn');
      const banners = document.querySelectorAll('#map-area-2 .t-banner');

      // Κυκλική εναλλαγή των 5 Σταδίων
      STATE.mapStage = (STATE.mapStage % 5) + 1;
      
      // Κλείνουμε πάντα το "ΜΟΝΟ ΤΕΡΑΤΑ" εκτός αν είμαστε στο Στάδιο 3
      if (monstersBtn) monstersBtn.style.display = (STATE.mapStage === 3) ? 'block' : 'none';
      if (STATE.mapStage === 3 && monstersBtn) monstersBtn.innerHTML = 'ΜΟΝΟ ΤΕΡΑΤΑ';
      STATE.monstersOnlyActive = false;
  ...
  return id && CONFIG.groups.B.includes(id) && !b.classList.contains('mythic-banner') && !b.classList.contains('eco-banner');
          ...
          break;
        case 2: // Όλα τα κανονικά
          MapEngine.filterBanners(banners, b => !b.classList.contains('mythic-banner') && !b.classList.contains('eco-banner'));
          MapEngine.toggleSVGs(CONFIG.svg.monsters, false);
          MapEngine.toggleSVGs(CONFIG.svg.eco, false);
          if (btn) btn.innerHTML = "⚡ Μυθική Πιερία!";
          break;
        case 3: // Μόνο Μυθικά
          MapEngine.filterBanners(banners, b => b.classList.contains('mythic-banner'));
          MapEngine.toggleSVGs(CONFIG.svg.monsters, true);
          MapEngine.toggleSVGs(CONFIG.svg.eco, false);
          if (btn) btn.innerHTML = "🌿Τα Μυστικά της Φύσης!";
          break;
        case 4: // Μόνο Οικολογικά
          MapEngine.filterBanners(banners, b => b.classList.contains('eco-banner'));
          MapEngine.toggleSVGs(CONFIG.svg.monsters, false);
          MapEngine.toggleSVGs(CONFIG.svg.eco, true);
 ...
 const id = (b.dataset.action?.match(/showPlace\('([^']+)'/) || [])[1];
  return id && CONFIG.groups.A.includes(id) && !b.classList.contains('mythic-banner') && !b.classList.contains('eco-banner');
    ...
    toggleMonstersOnly: () => {
      ...
      STATE.monstersOnlyActive = !STATE.monstersOnlyActive;
      
      if (!STATE.monstersOnlyActive) {
        // --- Η ΔΙΟΡΘΩΣΗ ---
        // Επαναφορά: Θέλουμε να εμφανιστούν ΟΛΑ τα banners που ανήκουν στη Μυθική Πιερία
        MapEngine.filterBanners(banners, b => b.classList.contains('mythic-banner'));
        btn.innerHTML = "ΜΟΝΟ ΤΕΡΑΤΑ";
      } else {
        // Μόνο Τέρατα: Κρύβουμε τα απλά μυθικά pins (π.χ. Μούσες) και κρατάμε μόνο τα τέρατα
       MapEngine.filterBanners(banners, b => CONFIG.monsterPlaceIDs.some(id => b.dataset.action?.includes(id)));
        MapEngine.toggleSVGs(CONFIG.svg.monsters, true); // Σιγουρεύουμε ότι τα τέρατα κινούνται
        btn.innerHTML = "ΕΠΑΝΑΦΟΡΑ";
      }
    },

    filterBanners: (banners, conditionFn) => {
      banners.forEach(b => {
        const isVisible = conditionFn(b);
        b.classList.toggle('visible-group', isVisible);
        b.classList.toggle('hidden-group', !isVisible);
      });
    },

    toggleSVGs: (svgArray, isActive) => {
      svgArray.forEach(id => {
    ...
    applyAtmosphere: (placeId) => {
      const overlay = document.getElementById('map-dark-overlay');
      if (!overlay) return;
      
    for (const [cssClass, places] of Object.entries(CONFIG.atmospheres)) {
        if (places.includes(placeId)) {
          overlay.classList.add(cssClass);
          // Το break αφαιρέθηκε! Τώρα μια τοποθεσία μπορεί να δεχθεί πολλαπλά εφέ.
        }
      }
    }
  };
  ...
  // ==========================================
  // 5. TOOLTIP & ΓΡΙΦΟΙ (The Game Logic)
  // ==========================================
  const TooltipEngine = {
    show: (placeId, bannerEl) => {
      const data = window.MYTHIC_DB[placeId];
      if (!data) return;

      if (bannerEl) bannerEl.classList.add('visited-banner');

      document.getElementById('tt-title').innerHTML = data.title;
      document.getElementById('tt-desc').innerHTML = data.desc;
      ...
      // Λογική Κρυμμένου Θησαυρού
      if (placeId === 'peristasi') {
        const t1 = document.getElementById('treasure-2-1');
        if (t1 && t1.style.display !== 'none') t1.classList.remove('hidden-treasure');
      }

      AudioEngine.playPin(data.sound);
      MapEngine.applyAtmosphere(placeId);
      document.getElementById('tolkien-tooltip').classList.add('active');
    },

   hide: () => {
      // Σταματάμε τον ήχο της τοποθεσίας εφόσον ο χρήστης έκλεισε την κάρτα της
      if (STATE.audioCurrentPin) {
        STATE.audioCurrentPin.pause();
        STATE.audioCurrentPin.currentTime = 0;
      }
      
      const tooltip = document.getElementById('tolkien-tooltip');
      if (tooltip) tooltip.classList.remove('active');
     const overlay = document.getElementById('map-dark-overlay');
      if (overlay) overlay.classList.remove(...Object.keys(CONFIG.atmospheres));
    },

    handleTreasure: (step) => {
      AudioEngine.stopAll();
      const texts = {
        1: { t: "📜 Παλιά Περγαμηνή", d: "Τα κατάφερες! Η περγαμηνή γράφει:<br><br><i>«Ο επόμενος θησαυρός είναι ένα αρχαίο νόμισμα. Κρύβεται στα βόρεια, δίπλα σε ένα σπουδαίο αρχαίο λιμάνι, εκεί όπου βρέθηκαν τα πρώτα δείγματα της ελληνικής γραφής...»</i>" },
        2: { t: "🪙 Αρχαίο Νόμισμα", d: "Υπέροχα! Ταξίδεψες μέχρι την Αρχαία Μεθώνη και βρήκες το νόμισμα! Πάνω του έχει χαραγμένο το τελευταίο στοιχείο:<br><br><i>«Το τελικό έπαθλο φυλάσσεται τέρμα νότια, δίπλα στα μεγάλα πέτρινα τείχη του κάστρου που βρέχονται από το Αιγαίο...»</i>" },
        3: { t: "💍 Το Δαχτυλίδι της Πιερίας!", d: "<h3 style='color:#c29b38; margin: 0;'>ΣΥΓΧΑΡΗΤΗΡΙΑ!</h3><br>Ανακάλυψες τον μεγάλο θησαυρό, έλυσες όλους τους γρίφους και ολοκλήρωσες την περιπέτεια με επιτυχία!" }
      };
      
      document.getElementById('tt-title').innerHTML = texts[step].t;
    document.getElementById('tt-desc').innerHTML = texts[step].d;
      document.getElementById('tt-img').style.display = 'none';
      document.getElementById('tolkien-tooltip').classList.add('active');

      // Αφαιρέθηκε το style.display = 'none' ώστε ο θησαυρός να παραμένει πατήσιμος 
      // στην περίπτωση που ο χρήστης κλείσει την καρτέλα κατά λάθος!
    // Προσθήκη Optional Chaining (?.) για αποτροπή κατάρρευσης αν το επόμενο στοιχείο δεν υπάρχει
      if (step < 3) document.getElementById(`treasure-2-${step+1}`)?.classList.remove('hidden-treasure');
    }
  };

  // ==========================================
  // 6. ΠΡΟΕΤΟΙΜΑΣΙΑ DOM & PANZOOM (PRO SENIOR VERSION)
  // ==========================================
  const CoreSystem = {
    init: async () => { 
      
   // 1. Καθαρισμός HTML
      // Προσθήκη του περιέκτη της καρτέλας (#tolkien-tooltip) για να μετατραπεί σωστά
   // 1. Καθαρισμός HTML
    // 1. Καθαρισμός HTML
      // Προσθήκη του περιέκτη της καρτέλας (#tolkien-tooltip) για να μετατραπεί σωστά
      document.querySelectorAll('#map-area-2 [onclick], #tolkien-tooltip [onclick], #sound-btn-2, #toggle-btn-2, #monsters-only-btn').forEach(el => {
        const clickAction = el.getAttribute('onclick');
        // Ασφάλεια: Προχωράμε στη μεταφορά ΜΟΝΟ αν υπήρχε όντως εντολή onclick
        if (clickAction) {
          el.dataset.action = clickAction; 
          el.removeAttribute('onclick'); 
        }
      });
     // 2. Έξυπνο Event Delegation
      document.addEventListener('click', (e) => {
        if (STATE.isDraggingMap) return; // Η προστασία ΠΡΕΠΕΙ να ελέγχεται ΠΡΩΤΗ για να μην κλείνει η καρτέλα άδικα!

      const target = e.target.closest('[data-action]');
        const actionStr = target ? (target.dataset.action || "") : "";
        
        // Αποτροπή κλεισίματος της καρτέλας αν πατηθεί ένα από τα κεντρικά κουμπιά ελέγχου
        const isUIAction = actionStr.includes('toggleMute2') || actionStr.includes('toggleBanners2') || actionStr.includes('showOnlyMonsters');

        const tooltip = document.getElementById('tolkien-tooltip');
        if (tooltip && tooltip.classList.contains('active') && !tooltip.contains(e.target) && !isUIAction) {
          TooltipEngine.hide();
        }
        ...
        const isWidgetAction = ['toggleMute2', 'toggleBanners2', 'showOnlyMonsters', 'hidePlace', 'foundTreasure2', 'showPlace'].some(cmd => actionStr.includes(cmd));
        
        // Προστασία του ιστότοπου: επιτρέπουμε να λειτουργήσουν τα κλικ που ΔΕΝ αφορούν τον δικό μας χάρτη
        ...
        // Router εντολών
        if (actionStr.includes('toggleMute2')) AudioEngine.toggleMute();
        else if (actionStr.includes('toggleBanners2')) MapEngine.cycleState();
        else if (actionStr.includes('showOnlyMonsters')) MapEngine.toggleMonstersOnly();
        else if (actionStr.includes('hidePlace')) TooltipEngine.hide();
        else if (actionStr.includes('foundTreasure2')) {
          const stepMatch = actionStr.match(/foundTreasure2\((\d+)/);
          if (stepMatch) TooltipEngine.handleTreasure(parseInt(stepMatch[1], 10));
        }
        else if (actionStr.includes('showPlace')) {
          const placeMatch = actionStr.match(/showPlace\('([^']+)'/);
          if (placeMatch) TooltipEngine.show(placeMatch[1], target);
        }
      });

      // --- [ΔΙΟΡΘΩΣΗ PERFORMANCE: Εκτελούνται ΠΡΙΝ το κατέβασμα του Panzoom] ---
      // 3. Preloading εικόνων (Background Task)
      window.addEventListener('load', () => {
        Object.values(window.MYTHIC_DB || {}).forEach(place => {
          if (place.img) new Image().src = place.img;
        });
      }, { passive: true });

      // 4. Αρχική Φόρτωση του Χάρτη (Ακαριαία εμφάνιση της σωστής κατάστασης)
      setTimeout(() => MapEngine.cycleState(true), 300);


      // 5. Δυναμική Φόρτωση Panzoom (Δεν μπλοκάρει πλέον τα προηγούμενα)
      ...
      if (isDesktop && mapAreaPanzoom) {
        ...
        // Δημιουργία κοινής κλάσης εξαίρεσης ΚΑΙ για Banners ΚΑΙ για Treasures
          document.querySelectorAll('#map-area-2 .t-banner, #map-area-2 .t-treasure').forEach(el => el.classList.add('pz-ignore'));

          // Η βιβλιοθήκη κατέβηκε, την ενεργοποιούμε
          const panzoom = Panzoom(mapAreaPanzoom, {
            maxScale: 2.5, minScale: 1, contain: 'none', step: 0.3, pinchSpeed: 1, excludeClass: 'pz-ignore'
          });
          
          if (zoomWrapper) zoomWrapper.addEventListener('wheel', panzoom.zoomWithWheel, { passive: false });

          // Drag Logic ΜΟΝΟ για Desktop (Στο κινητό το αναλαμβάνει το OS)
          let startX, startY;
          mapAreaPanzoom.addEventListener('pointerdown', (e) => {
            startX = e.clientX; startY = e.clientY; STATE.isDraggingMap = false;
         ...
         mapAreaPanzoom.addEventListener('pointerup', (e) => {
            ...
            if (Math.abs(e.clientX - startX) > 7 || Math.abs(e.clientY - startY) > 7) {
              STATE.isDraggingMap = true;
              // Απελευθερώνουμε το UI (ξεκλειδώνουμε τα κουμπιά) αφού αποτραπεί το λανθασμένο κλικ
              setTimeout(() => { STATE.isDraggingMap = false; }, 50);
            }
          }, { passive: true });

       mapAreaPanzoom.addEventListener('panzoomstart', () => {
            document.querySelectorAll('#map-area-2 .t-banner, #map-area-2 .t-treasure').forEach(el => el.style.pointerEvents = 'none');
          });
         mapAreaPanzoom.addEventListener('panzoomend', () => {
            setTimeout(() => {
              // Χρησιμοποιούμε κενό string ('') για να αναιρεθεί το inline style, αφήνοντας τις CSS κλάσεις να κρύβουν σωστά τα στοιχεία!
              document.querySelectorAll('#map-area-2 .t-banner, #map-area-2 .t-treasure').forEach(el => el.style.pointerEvents = '');
            }, 50);
          });

          console.info("[Rima] Desktop: Panzoom loaded dynamically.");

        } catch (err) {
          console.error("[Rima] Το Panzoom απέτυχε να φορτώσει:", err);
        }
      } else {
        // --- MOBILE/TABLET MODE ---
        const rimaContainer = document.getElementById('rima-container');
        if (rimaContainer) rimaContainer.classList.add('rima-mobile-fallback');
        console.info("[Rima] Mobile/Touch detected: Panzoom skipped.");
      }
    }
  };

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', CoreSystem.init);
  else CoreSystem.init();
