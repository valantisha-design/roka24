...
mythic_echidna: { 
  ...
  /* ΔΙΑΓΡΑΦΗ: Αφαιρέστε εντελώς όλο το παραπάνω μπλοκ του `setTimeout`. 
         Η θάλασσα πρέπει να συνεχίζει να παίζει αρμονικά στο υπόβαθρο. */
 playPin: (url) => {
      if (!STATE.isSoundOn) return;
  ...
  const TooltipEngine = {
   ...
   hide: () => {
    ...
    const tooltip = document.getElementById('tolkien-tooltip');
      ...
      // ΠΡΟΣΘΗΚΗ: Επαναφέρουμε την ατμόσφαιρα της θάλασσας όταν κλείσει η καρτέλα
      AudioEngine.playOcean();
    },
