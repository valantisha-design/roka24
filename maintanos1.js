...
mythic_echidna: { 
  ...
  // ==========================================
  // 6. ΠΡΟΕΤΟΙΜΑΣΙΑ DOM & PANZOOM
  // ==========================================
  // ==========================================
  // 6. ΠΡΟΕΤΟΙΜΑΣΙΑ DOM & PANZOOM (DYNAMIC)
  // ==========================================
  const CoreSystem = {
    init: async () => { // Το κάναμε async για το δυναμικό Panzoom
      
      // 1. Καθαρισμός HTML (Αφαιρούμε τα onclick δυναμικά για να μην υπάρχουν Errors)
      document.querySelectorAll('#map-area-2 [onclick], #sound-btn-2, #toggle-btn-2, #monsters-only-btn').forEach(el => {
        el.dataset.action = el.getAttribute('onclick'); // Αποθήκευση της παλιάς εντολής
        el.removeAttribute('onclick'); // Διαγραφή για καθαρό DOM
      });

      // 2. Έξυπνο Event Delegation (ΜΟΝΟ ΕΝΑΣ LISTENER για όλο τον χάρτη)
      document.addEventListener('click', (e) => {
        ...
        // Αν μετακινούμε τον χάρτη (Drag), αγνοούμε το Κλικ!
        if (STATE.isDraggingMap) return;

        // Βρίσκουμε αν πατήθηκε στοιχείο που είχε onclick
        ...
        // Router εντολών (Διαβάζει το string και εκτελεί την αντίστοιχη συνάρτηση)
      ...
      // 3. Δυναμική Αρχικοποίηση Panzoom (Ο έλεγχος που φτιάξαμε)
      ...
      if (isDesktop && mapAreaPanzoom) {
        // --- DESKTOP MODE: Φορτώνουμε και στήνουμε το Panzoom ---
        try {
          ...
          // Τα δικά σου events για το Dragging (για να μην ανοίγουν τα tooltips όταν σέρνεις)
          let startX, startY;
          ...
          mapAreaPanzoom.addEventListener('pointerup', (e) => {
            ...
            if (Math.abs(e.clientX - startX) > 7 || Math.abs(e.clientY - startY) > 7) STATE.isDraggingMap = true;
          }, { passive: true });
        ...
        console.info("[Rima] Mobile/Touch detected: Panzoom skipped. Native scroll enabled.");
      }

      // 4. Preloading εικόνων (Στο background)
      ...
      // 5. Αρχική Φόρτωση (Με την ασφαλή καθυστέρηση των 300ms)
