...
mythic_echidna: { 
  ...
  const CoreSystem = {
    init: () => {
      ...
      // 1. Καθαρισμός HTML (Στοχεύουμε ΑΥΣΤΗΡΑ τον 2ο Χάρτη για να μην χαλάσει ο 1ος!)
      ...
      document.addEventListener('click', (e) => {
        ...
        if (actionStr.includes('toggleMute2')) AudioEngine.toggleMute();
          ...
          if (placeMatch) TooltipEngine.show(placeMatch[1], target.closest('.t-banner'));
        }
      });

      // 3. Αρχικοποίηση Panzoom
      const mapAreaPanzoom = document.getElementById('map-area-2');
      const zoomWrapper = mapAreaPanzoom?.parentElement;
      
      if (mapAreaPanzoom && typeof Panzoom !== 'undefined') {
        ...
        mapAreaPanzoom.addEventListener('panzoomend', () => {
          ...
          }, 50); // Μικρή καθυστέρηση για αποφυγή ghost-clicks
        });
      }
