...
mythic_echidna: { 
  ...
  const AudioEngine = {
    ...
    playOcean: () => {
      ...
      STATE.audioTimers.stop = setTimeout(() => {
        STATE.audioTimers.fade = setInterval(() => {
          if (STATE.audioOcean.volume > 0.1) STATE.audioOcean.volume -= 0.1;
   ...
   playPin: (url) => {
      if (!STATE.isSoundOn) return;
      AudioEngine.stopAll(); // Πρέπει ΠΑΝΤΑ να σταματάμε τον προηγούμενο ήχο
      if (!url) return;      // Διακοπή εάν η νέα τοποθεσία δεν έχει δικό της ήχο

      STATE.audioCurrentPin = new Audio(url);
 ...
 const id = (b.dataset.action?.match(/showPlace\('([^']+)'/) || [])[1];
  return id && CONFIG.groups.A.includes(id) && !b.classList.contains('mythic-banner') && !b.classList.contains('eco-banner');
    ...
    applyAtmosphere: (placeId) => {
      ...
      overlay.className = 'map-overlay'; // Reset
  ...
  const CoreSystem = {
    init: async () => { 
      ...
      document.querySelectorAll('#map-area-2 [onclick], #sound-btn-2, #toggle-btn-2, #monsters-only-btn').forEach(el => {
        el.dataset.action = el.getAttribute('onclick'); 
      ...
      if (isDesktop && mapAreaPanzoom) {
        try {
          ...
          const panzoom = Panzoom(mapAreaPanzoom, {
            maxScale: 2.5, minScale: 1, contain: 'none', step: 0.3, pinchSpeed: 1, excludeClass: 't-banner'
          });
