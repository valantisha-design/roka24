@import url('https://fonts.googleapis.com/css2?family=Caveat:wght@600&family=Poppins:wght@300;500;600&display=swap');    /* από τη χρονοκαψουλα  */

 

/* ===== BOOSTED MENU + SLIDER (φαίνεται καθαρά) ===== */
...
.post-outer {
  opacity: 0;
  transform: scale(0.05) translateZ(-2500px);
  animation: zoomFromDepth 3.5s cubic-bezier(.17, .67, .43, 1.2) forwards;
...
@media screen and (min-width: 768px) {
    .main-inner {
        ...
        padding-bottom: 8200px !important;
    }
}
...
@media screen and (min-width: 800px) and (max-width: 1800px) and (pointer: coarse) {
  nav {
    gap: 19px !important; 
  }
...
/* Το κεντρικό πλέγμα του Widget */
  ...
  /* --- Ρυθμίσεις για τη διπλή κάρτα (4η) --- */
  ...
  @media screen and (max-width: 768px) {
    .school-stats-wrapper {
      ...
      gap: 12px;
      margin-top: 2px;    /* Απόσταση από πάνω */
...
/* Αποστάσεις για υπολογιστές (PC) */
.stats-notice {
  margin-top: 5px;
  margin-bottom: -30px;
}

/* Αποστάσεις για κινητά και τάμπλετ (768px και κάτω) */
@media screen and (max-width: 768px) {
  .stats-notice {
    margin-top: 1px;    /* 👈 Άλλαξε αυτό όσο θες για το top στα κινητά */
    margin-bottom: 2px;  /* 👈 Άλλαξε αυτό όσο θες για το bottom στα κινητά */
  }
}
.glass-modal-overlay {
  position: fixed;
  top: 0; left: 0; width: 100vw; height: 100vh;
  background: transparent; /* 👈 ΑΛΛΑΓΗ: Έγινε διάφανο, έφυγε το σκοτείνιασμα! */
  display: flex; align-items: center; justify-content: center;
  opacity: 0; visibility: hidden;
  transition: all 0.3s ease;
  z-index: 2147483647;
...
.glass-modal-box {
  ...
  padding: 15px 5px;
  text-align: center;
  max-width: 370px; width: 85%;
  box-shadow: 0 10px 30px rgba(0, 0, 0, 0.6);
  transform: translateY(-30vh) scale(0.95); /* Σηκώνει την κάρτα 15% πιο ψηλά */
  transition: all 0.3s ease;
  font-family: 'Inter', Arial, sans-serif;
  position: relative;
}
...
.glass-modal-overlay.active .glass-modal-box {
  transform: translateY(-30vh) scale(1); /* Κρατάει την κάρτα ψηλά όταν ανοίγει */
}
...
.glass-modal-close:hover { color: #f87171; }

.glass-modal-icon { font-size: 50px; margin-bottom: 12px; }

.glass-modal-text {
  font-size: 16px; color: #f1f5f9; /* Ανοιχτόχρωμα γράμματα για το σκούρο φόντο */
  margin-bottom: 25px; line-height: 1.6; font-weight: 500;
}

.glass-modal-btn {
  display: inline-block; padding: 12px 28px;
  background: #a90e0e; /* Το κόκκινο του σχολείου */
  color: #fff !important; text-decoration: none;
  border-radius: 12px; font-weight: 800; text-transform: uppercase;
  letter-spacing: 1px; box-shadow: 0 4px 15px rgba(169, 14, 14, 0.4);
  transition: background 0.3s, transform 0.2s;
}
.glass-modal-btn:hover {
  background: #c91414; transform: translateY(-2px);
}

/* Το Modal Overlay πάντα γεμίζει την οθόνη */
.glass-modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
  ...
  z-index: 999999; /* Θα είναι πάντα μπροστά από όλα */
padding-top: 250px;
}
...
.glass-modal-box {
  width: 85%;
  max-width: 370px;
  /* Αφαίρεσε τα margin-left που είχες στα media queries */
  margin: 0 auto; 
...
@media screen and (min-width: 900px) and (max-width: 1100px) {
  .glass-modal-overlay {
    ...
    /* Ρύθμιση θέσης: flex-start (πάνω), center (μέση), flex-end (κάτω) */
    align-items: flex-start;   
    /* Ρύθμιση οριζόντιας θέσης: flex-start (αριστερά), center (κέντρο), flex-end (δεξιά) */
    justify-content: flex-start;   
    
    /* Ακριβείς ρυθμίσεις για τη θέση του παραθύρου */
    padding-top: 1100px;        
    padding-left: 250px;         
  }

  .glass-modal-box {
    margin: 0 !important;      /* Μηδενισμός περιθωρίων για ευθυγράμμιση */
    width: 700px !important;   /* Το σταθερό πλάτος */
  }
}
...
/* ΠΡΟΣΘΗΚΗ user-select για αποφυγή επιλογής κειμένου όταν τα παιδιά "κλικάρουν" γρήγορα */
.glass-bell-tracker {
    user-select: none;
    -webkit-user-select: none;
    /* (τα υπόλοιπα τα αφήνεις ως έχουν) */
    background: rgba(255, 255, 255, 0.18);
    backdrop-filter: blur(12px);
    -webkit-backdrop-filter: blur(12px);
    border: 1px solid rgba(255, 255, 255, 0.3);
...
/* ΑΛΛΑΓΗ για Hardware Acceleration (GPU) */
.bouncing-secret {
    display: inline-block; 
    animation: bounce-jump 2.5s infinite; 
    cursor: pointer; 
    will-change: transform; /* Οδηγία στην κάρτα γραφικών του κινητού */
}
...
/* ΑΛΛΑΓΗ (box-shadow αντί για filter) για εξοικονόμηση μπαταρίας */
.live-dot {
    width: 10px;
    height: 10px;
    ...
    border-radius: 50%;
    margin-right: 8px;
    box-shadow: 0 0 4px #f22c2c, 0 0 2px #ffffff;
    animation: pulse-theme-red 1.8s infinite ease-in-out;
}

.live-dot.paused {
    background-color: #7f8c8d;
    animation: none;
    box-shadow: none; /* Σβήνει η λάμψη στις αργίες */
}

@keyframes pulse-theme-red {
    0% {
      transform: scale(0.9);
      box-shadow: 0 0 3px #f22c2c; 
    }
    50% {
      transform: scale(1.25);
      box-shadow: 0 0 8px #f22c2c, 0 0 2px #ffffff;
    }
    100% {
      transform: scale(0.9);
      box-shadow: 0 0 3px #f22c2c;
