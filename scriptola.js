...
(() => {
    ...
    const CONFIG = Object.freeze({
        API: {
            FEEDBACK: "https://script.google.com/macros/s/AKfycbx2gHWrrA3A0MjsQLz30e3NFSpr6BzorfRf08ZR_v5of87VcgQjNSJJ_Re0ivyZYcLTxA/exec"
        },
        SELECTORS: {
    ...
    class FeedbackEngine {
        static scan() {
            document.querySelectorAll(CONFIG.SELECTORS.ARTICLE_CONTAINER).forEach(container => {
                ...
                // Δεν χρησιμοποιούμε onclick="" πλέον!
                div.innerHTML = `
                    <div class="smart-feedback-buttons" style="${btnStyle}" data-postid="${postId}">
                ...
                fetch(`${CONFIG.API.FEEDBACK}?postId=${encodeURIComponent(postId)}&nocache=${Date.now()}`)
                    .then(r => r.json())
                    .then(d => {
        ...
        static handleReaction(btn) {
            ...
            fetch(`${CONFIG.API.FEEDBACK}?postId=${encodeURIComponent(postId)}&emoji=${type}`)
                .then(r => r.json())
                .then(d => {
                    localStorage.setItem('feedback_' + postId, 'voted');
                    span.innerText = d[type];
